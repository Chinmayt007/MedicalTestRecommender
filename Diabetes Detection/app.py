...
st.sidebar.write(
    'Predict whether you have diabetes or not by entering the parameters. The results are located at the bottom of the page')
option = st.sidebar.selectbox(
    'Select your Machine Learning Model', ('K Nearest Neighbors', 'Logistic Regression'))
...
def getUserInfo():
    ...
    glucose = st.sidebar.slider(
        'Plasma Glucose Concentration (mg/dl)', 0, 199, 117)
    bloodPressure = st.sidebar.slider(
        'Diastolic Blood Pressure (mm Hg)', 0, 122, 72)
    skinThickness = st.sidebar.slider(
        'Triceps skin fold thickness (mm)', 0, 99, 23)
...
if prediction == 1:
    st.warning(
        "You have a probability of having diabetes. Please consult with your doctor")
elif prediction == 0:
...
st.markdown(
    f"<center> You have an <b>{round(predictionProbability[0]*100)}%</b> chance of having diabetes </center>", unsafe_allow_html=True)


################################################################################################################################################################################################################
# Render the Footer, contained in a frame of size 200x200.
components.html("<html><body>©️Diabities Prediction System Made With &#10084 By <a href='https://github.com/Chinmayt007'>Chinmay Tiwari</a> In India</h3></body></html>", width=10000, height=250)
################################################################################################################################################################################################################
